...
class _DestinationDetailScreenState extends State<DestinationDetailScreen> {
        ...
        SliverAppBar(
          ...
          flexibleSpace: FlexibleSpaceBar(
            title: Text('${dest.name}, ${dest.country}',
              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, shadows: [Shadow(blurRadius: 4, color: Colors.black54)])),
            background: dest.imageAssets.isEmpty
                ? Container(color: theme.colorScheme.primaryContainer,
                    child: Icon(Icons.travel_explore, size: 80, color: theme.colorScheme.primary))
                : Stack(children: [
                    PageView.builder(
                      itemCount: dest.imageAssets.length,
                      onPageChanged: (i) => setState(() => _imageIndex = i),
                      itemBuilder: (_, i) => Image.asset(dest.imageAssets[i], fit: BoxFit.cover,
                    ...
                    if (dest.imageAssets.length > 1)
                      Positioned(bottom: 48, left: 0, right: 0,
                        child: Row(mainAxisAlignment: MainAxisAlignment.center,
                          children: List.generate(dest.imageAssets.length, (i) => AnimatedContainer(
                            duration: const Duration(milliseconds: 200),
                            margin: const EdgeInsets.symmetric(horizontal: 3),
                            width: _imageIndex == i ? 16 : 8, height: 8,
        ...
        SliverToBoxAdapter(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              child: Text(dest.continent, style: TextStyle(color: theme.colorScheme.onPrimaryContainer, fontWeight: FontWeight.w600)),
            ),
            const SizedBox(height: 12),
            Text(dest.subtitle, style: theme.textTheme.titleMedium?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
            ...
            Row(children: [
              ...
              if (dest.longStayFriendly) ...[
                Icon(Icons.calendar_month, size: 16, color: theme.colorScheme.secondary),
                const SizedBox(width: 4),
            ...
            // Amenities checklist
            Text('Traveller Checklist', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            ..._amenityRows(dest, theme),

            const SizedBox(height: 24),
            // Top tags
            Text('Vibe Tags', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Wrap(spacing: 8, runSpacing: 8,
              children: (dest.tagAffinities.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
                  .take(8).map((e) => Chip(
                    label: Text('${e.key.replaceAll('_', ' ')} ·${e.value.toInt()}',
            ...
            const SizedBox(height: 24),
            // Mini itinerary
            Text('Sample 3-Day Idea', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
  ...
  List<String> _itinerary(String destName) {
    ...
    final custom = <String, List<String>>{
      'Tokyo': ['Day 1 — Shibuya crossing, Harajuku & ramen dinner','Day 2 — Senso-ji temple, TeamLab & sushi','Day 3 — Shinjuku gardens, Akihabara & yakitori'],
      'Bali': ['Day 1 — Arrive Ubud, rice terrace walk & sunset temple','Day 2 — Sacred Monkey Forest, spa & traditional dance','Day 3 — Tanah Lot & Seminyak beach'],
      'Barcelona': ['Day 1 — La Sagrada Família, La Rambla & tapas bar hop','Day 2 — Park Güell, Gothic Quarter & sunset at Barceloneta','Day 3 — Montjuïc, El Born market & flamenco night'],
    };
